...
class LDAM_loss(loss_function):
    ...
    def __call__(self, y_true, y_pred):
        ...
        batch_m = np.where(y_true == 0, self.m_list[0], self.m_list[1])

        y_m = y_pred - batch_m

        # if condition is true, return x_m[index], otherwise return x[index]
        output = np.where(y_true > 0, y_m, y_pred)
        output = output + self.init_score

        p = 1/(1+np.exp(-output)) + 1e-12  # For numerical stability
        pos_loss = np.log(p) * self.alpha_m
        neg_loss = np.log(1-p) * self.alpha_M
